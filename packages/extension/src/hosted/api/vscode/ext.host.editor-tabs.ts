...
export class ExtHostEditorTabs implements IExtHostEditorTabs {
  ...
  private readonly _proxy: IMainThreadEditorTabsShape;
  ...
  constructor(rpcProtocol: IRPCProtocol) {
    ...
    this._proxy.$initializeState();
  }

  get tabs(): IEditorTab[] {
    return this.tabGroups.all.reduce<IEditorTab[]>(
      (prev, next) =>
        prev.concat(
          next.tabs.map(
            (t) =>
              ({
                group: t.group.viewColumn + 1,
                isActive: t.isActive,
                name: t.label,
                resource: (t.input as any)?.uri,
              } as IEditorTab),
          ),
        ),
      [],
  ...
  onDidChangeTabs: Event<void> = Event.map(this._onDidChangeTabs.event, () => void 0);
