...
export class SingleUseTestCollection extends Disposable {
  private readonly debounceSendDiff = this.registerDispose(new RunOnceScheduler(() => this.flushDiff(), 200));
  private readonly diffOpEmitter = this.registerDispose(new Emitter<TestsDiff>());
  ...
  private upsertItem(actual: TestItemRaw, parent: OwnedCollectionTestItem | undefined) {
    if (!(actual instanceof TestItemImpl)) {
      throw new Error(
        `TestItems provided to the VS Code API must extend \`vscode.TestItem\`, but ${actual.id} did not`,
      );
    }
  ...
  private diffTagRefs(newTags: ITestTag[], oldTags: ITestTag[], extId: string) {
    ...
    this.pushDiff([
      TestDiffOpType.Update,
      {
        extId,
        item: {
          tags: newTags.map((v) => Convert.TestTag.namespace(this.controllerId, v.id)),
        },
      },
    ]);
  ...
  private connectItem(
    ...
    parent: OwnedCollectionTestItem | undefined,
  ) {
    this.setItemParent(actual, parent);
