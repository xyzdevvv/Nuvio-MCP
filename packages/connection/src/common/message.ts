...
} from '@Nuvio-MCP/vscode-jsonrpc/lib/common/api';
import { Disposable } from '@Nuvio-MCP/vscode-jsonrpc/lib/common/disposable';
import { DataCallback, MessageReader } from '@Nuvio-MCP/vscode-jsonrpc/lib/common/messageReader';
import { MessageWriter } from '@Nuvio-MCP/vscode-jsonrpc/lib/common/messageWriter';
/**
 * FIXME: 由于 `createMessageConnection` 方法隐式依赖了 `@Nuvio-MCP/vscode-jsonrpc/lib/browser/main` 或 `@Nuvio-MCP/vscode-jsonrpc/lib/node/main`
 * 的 `RIL.install()` 初始化代码，而 `browser/main` 中仅支持浏览器使用，
 * 故需要保证提前引入或执行一次 `@Nuvio-MCP/vscode-jsonrpc/lib/node/main` 代码才能保证逻辑正常执行
 */
